...
#define MTC_W25X40_DW         (0x4014) /* W25Q32DW */
...
rt_size_t w25qxx_flash_read(rt_device_t dev,
                                   rt_off_t pos,
                                   void* buffer,
                                   rt_size_t size)
{
...
rt_size_t w25qxx_flash_write(rt_device_t dev,
                                    rt_off_t pos,
                                    const void* buffer,
                                    rt_size_t size)
{
...
rt_err_t w25qxx_init(const char * flash_device_name, const char * spi_device_name)
{
    ...
    /* init flash */
    {
        ...
        if(memory_type_capacity == MTC_W25X40_DW)
        {
            FLASH_TRACE("W25X40 detection\r\n");
